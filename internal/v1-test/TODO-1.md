...
|---|---|---|---|---|---|
| [x] Critical: Record the current frontend/backend contracts | Prevent accidental API breakage while persistence is replaced. | Establishes a verifiable baseline before schema changes. | Inventory every route from `backend/internal/api/routes.go`, every response type from `frontend/src/api.ts`, and every mutable in-memory map in `backend/internal/api` and `backend/internal/platform`. Add a short contract section below this roadmap if an undocumented field is discovered. Do not change behavior in this item. | Run `cd backend && go test ./...`; run `cd frontend && npm run typecheck && npm test -- --run && npm run build`. Record baseline counts. | pending — backend `go test ./...`: PASS (9 packages); frontend typecheck/tests/build: PASS (45 tests, 29 files). |
