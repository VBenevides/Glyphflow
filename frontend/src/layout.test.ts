...
describe('UI layout contracts', () => {
  ...
  it('keeps shared surfaces hierarchical and usable on small screens', () => {
    ...
    expect(css).toContain('.gf-dialog-backdrop { position: fixed;')
    expect(css).toContain('background: rgb(5 12 25 / 68%); backdrop-filter: blur(4px);')
    ...
    expect(css).toContain('.gf-page-header-actions { justify-content: flex-start; width: 100%; }')
  })

  it('keeps the registry theme compact and lavender', () => {
    expect(css).toContain('--gf-page: oklch(0.985 0.004 280);')
    expect(css).toContain('.gf-app-shell { display: grid; grid-template-columns: 18rem 1fr;')
    expect(css).toContain('.gf-sidebar.is-collapsed .gf-nav-link.is-active')
    expect(css).toContain('.gf-pagination { display: flex; align-items: center; justify-content: flex-start; gap: 1rem; margin-top: 0.75rem; padding: 0; background: transparent; border: 0; border-radius: 0;')
    ...
    expect(css).toContain('.gf-content { width: 90%; max-width: none; margin: 0 auto; padding: clamp(1rem, 2.5vw, 1.75rem) 0; }')
    ...
    expect(css).toContain('.gf-metric { position: relative; display: grid; width: 100%; max-width: 32rem; box-sizing: border-box; justify-self: start;')
