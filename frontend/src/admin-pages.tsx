...
function IdentityAdminLayout({ view, title, description, action, refresh, children }: { view: IdentityView; title: string; description: string; action?: ReactNode; refresh?: ReactNode; children: ReactNode }) {
  const navigate = useNavigate()
  return <main className="gf-content"><PageHeader title={title} description={description} action={action} refresh={refresh} /><Tabs value={view} onValueChange={(next) => navigate(next === 'sso' ? '/admin/sso' : next === 'sessions' ? '/admin/users/sessions' : '/admin/users')}><TabsList aria-label="Identity administration"><TabsTrigger value="users">Users</TabsTrigger><TabsTrigger value="sessions">Sessions</TabsTrigger><TabsTrigger value="sso">SSO</TabsTrigger></TabsList></Tabs>{children}</main>
}
...
export function UserManagementPage({ view = 'users' }: { view?: IdentityView } = {}) {
  ...
  return <IdentityAdminLayout view={view} title="Users and sessions" description="Review identity methods, role sources, permissions, and active sessions." action={manage && <Button onClick={() => setCreating((value) => !value)}>{creating ? 'Cancel' : 'Create user'}</Button>} refresh={<QueryRefresh query={query} />}>
...
export function RoleManagementPage() {
  ...
  return <main className="gf-content"><PageHeader title="Roles and permissions" description="Seeded roles are immutable. Custom roles select from the application permission catalog." action={manage && <Button onClick={() => setEditing(null)}>Create role</Button>} refresh={<QueryRefresh query={query} />} />
...
export function SsoSettingsPage() {
  ...
  const [draft, setDraft] = useState({ key: '', name: '', issuer: '', clientId: '', secretReference: '' })
  ...
  const resetProvider = () => { setDraft({ key: '', name: '', issuer: '', clientId: '', secretReference: '' }); setGroupMappings([{ group: '', role: '' }]); setError('') }
  const closeProvider = () => { setCreating(false); resetProvider() }
  const addProvider = async (event: FormEvent) => {
    event.preventDefault(); setBusy(true); setError('')
    try {
      await api.post('/api/v1/admin/auth/providers', { key: draft.key.trim(), name: draft.name.trim(), issuer: draft.issuer.trim(), clientId: draft.clientId.trim(), secretReference: draft.secretReference.trim(), groupMapping: roleMappingsValue(groupMappings), enabled: true })
  ...
  return <IdentityAdminLayout view="sso" title="Single sign-on" description="Configure generic OIDC providers and group-to-role mappings. Resolved secrets are never shown." action={manage && <Button onClick={() => setCreating(true)}>Add provider</Button>} refresh={<QueryRefresh query={query} />}>
...
export function ExecutionStatusPage() {
  ...
  return <main className="gf-content"><PageHeader title="Execution Status" description="Exit codes reported by completed task processes." action={manage && !formOpen && <Button onClick={create}>Create exit code</Button>} refresh={<QueryRefresh query={query} />} />
