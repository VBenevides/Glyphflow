...
export function UserDetailsPage({ userId, self }: { userId: string; self: boolean }) {
  ...
  return <main className="gf-content"><PageHeader title="User details" description={self ? 'Your account details are read-only here.' : 'Review identity, access, and session details.'} action={<Link className="gf-button gf-button-secondary" title="Return to the users list" to="/admin/users">Back to users</Link>} /><QueryState query={query}>{(user) => <>
    ...
    <section className="gf-card-panel"><h2>Sessions</h2>{user.sessions?.length ? <ul className="gf-dashboard-list">{user.sessions.map((session) => <li key={session.id}><span>{session.id}<br /><small>{session.expiresAt ? formatDateTime(session.expiresAt) : 'Expiry unavailable'}</small></span></li>)}</ul> : <p className="gf-muted">No active sessions.</p>}</section>
