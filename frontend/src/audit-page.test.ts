...
describe('audit filters', () => {
  it('sends server-side actor/action/target/result/correlation filters', () => {
    ...
    expect(auditQuery(filters, 2)).toMatchObject({ actor: 'u1', action: 'role.update', target: 'role:admin', result: 'success', correlation_id: 'c1', exclude_target: '/api/v1/audit', page: 2 })
