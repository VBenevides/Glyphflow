...
export function AuditPage() {
  ...
  return <main className="gf-content">
    <PageHeader title="Audit events" description="Trace security and scheduler changes with redacted values." refresh={<QueryRefresh query={query} />} />

    <QueryState query={query} empty="No audit events match these filters.">{(data) => <><div className="gf-metric-grid"><MetricCard label="Number of events" value={data.total ?? 0} detail="Events matching the filters" icon={Activity} /><MetricCard label="Number of failure events" value={data.failureCount ?? 0} detail="Filtered events with failure results" icon={CircleOff} /><MetricCard label="Number of write events" value={data.writeCount ?? 0} detail="Filtered POST, PUT, PATCH, and DELETE events" icon={ListChecks} /></div><div className="gf-filter-bar"><FilterInput label="Actor" options={auditOptions.flatMap((event) => [event.actorName, event.actor, event.actorEmail].filter((value): value is string => Boolean(value)))} value={filters.actor} onChange={(value) => update('actor', value)} /><FilterInput label="Action" options={auditOptions.flatMap((event) => event.action ? [event.action] : [])} value={filters.action} onChange={(value) => update('action', value)} /><FilterInput label="Target" options={auditOptions.flatMap((event) => event.target ? [event.target] : [])} value={filters.target} onChange={(value) => update('target', value)} /><label>Result<select className="gf-input" value={filters.result} onChange={(event) => update('result', event.target.value)}><option value="">All</option><option>success</option><option>failure</option><option>accepted</option></select></label><FilterInput label="Correlation ID" options={auditOptions.flatMap((event) => event.correlationId ? [event.correlationId] : [])} value={filters.correlation} onChange={(value) => update('correlation', value)} /><div className="gf-filter-checkboxes"><label><input type="checkbox" checked={filters.excludeAuditReads} onChange={(event) => { setFilters((current) => ({ ...current, excludeAuditReads: event.target.checked })); setPage(1) }} /> Hide audit reads</label><label><input type="checkbox" checked={filters.excludeRunLogs} onChange={(event) => { setFilters((current) => ({ ...current, excludeRunLogs: event.target.checked })); setPage(1) }} /> Hide run logs</label><label><input type="checkbox" checked={filters.excludeGet} onChange={(event) => { setFilters((current) => ({ ...current, excludeGet: event.target.checked })); setPage(1) }} /> Hide GET requests</label></div></div>{data.items.length ? <><DataTable className="gf-audit-table" caption="Audit events" rows={data.items} columns={[
      ...
      { key: 'actor', label: 'Actor', className: 'gf-cell-actor', render: (event) => { const name = event.actorName ?? event.actor; if (!event.actor) { return '—' }; if (canOpenUsers || event.actor === profile?.id) { return <Identifier id={event.actor} name={name} href={`/admin/users/${encodeURIComponent(event.actor)}`} copyLabel="Copy actor ID" /> }; return <Identifier id={event.actor} name={name} copyLabel="Copy actor ID" /> } },
