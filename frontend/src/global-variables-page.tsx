...
export function GlobalVariablesPage() {
  ...
  const manage = hasPermission(permissions, 'tasks.manage')
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!validGlobalVariableName(draft.name)) { setError('Use letters, numbers, and underscores.'); return }
    setBusy(true); setError('')
    try {
      if (editing) await api.put(`/api/v1/global-variables/${encodeURIComponent(editing.id)}`, draft)
  ...
  return <main className="gf-content"><PageHeader title="Global environment variables" description="Reusable non-secret values. Reference them as $(VAR_NAME) in supported task and schedule fields." action={manage && !editing && <Button onClick={() => setEditing({ id: '', name: '', value: '' })}>Create variable</Button>} />
