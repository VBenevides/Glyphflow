...
export function enrollmentPayload(runnerName: string, platform: string, architecture: string, poolId?: string, capacity = 10, embeddedNatsEndpoint = '', controlPlaneURL = '') {
  return { runner_name: runnerName.trim(), platform: platform.trim(), architecture: architecture.trim(), capacity, ...(poolId ? { pool_id: poolId.trim() } : {}), ...(embeddedNatsEndpoint.trim() ? { embedded_nats_endpoint: embeddedNatsEndpoint.trim() } : {}), ...(controlPlaneURL.trim() ? { control_plane_url: controlPlaneURL.trim().replace(/\/$/, '') } : {}) }
}
