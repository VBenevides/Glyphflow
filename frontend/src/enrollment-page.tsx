...
type EnrollmentPayloadOptions = { poolId?: string; capacity?: number; embeddedNatsEndpoint?: string; controlPlaneURL?: string; ui?: WorkerUI }

export function enrollmentPayload(runnerName: string, platform: string, architecture: string, { poolId, capacity = 10, embeddedNatsEndpoint = '', controlPlaneURL = '', ui = 'gui' }: EnrollmentPayloadOptions = {}) {
...
function artifactName(id: string, platform: string, architecture: string, ui: WorkerUI) {
  const uiSuffix = ui === 'gui' ? '' : `-${ui}`
  const extension = platform === 'windows' ? '.exe' : ''
  return `${id}-glyphflow-runner-${platform}-${architecture}${uiSuffix}${extension}`
}
