...
export function DashboardPage() {
  ...
  return <main className="gf-content"><PageHeader title="Overview" description="A live view of the scheduler resources you can access." meta="Live scheduler data" refresh={<QueryRefresh query={[...results, projectionQuery]} />} />{!warningKey && projection?.available && projection.conflicts?.length ? <section className="gf-card-panel gf-overview-conflict-notice" aria-live="polite"><h2>Scheduling conflicts detected</h2><p className="gf-form-error" role="alert">{projection.conflicts.length} exclusive-resource conflict{projection.conflicts.length === 1 ? '' : 's'} were found in the seven-day projection.</p>{projectionIsStale(projection.calculatedAt) && <p className="gf-stale-warning" role="status">This report is older than one hour; the last successful snapshot is shown.</p>}<ul>{projection.conflicts.slice(0, 5).map((conflict) => <li key={conflict.id}>{conflict.resourceName} · {conflict.occurrences.length} affected occurrence{conflict.occurrences.length === 1 ? '' : 's'}</li>)}</ul><div className="gf-dialog-actions"><Link className="gf-button gf-button-secondary" to="/schedules?tab=gantt">View Scheduling Gantt</Link></div></section> : null}<div className="gf-metric-grid">{metricWidgets.map((widget) => <WidgetState key={widget.key} widget={widget} result={resultFor(widget)} />)}</div><div className="gf-dashboard-grid">{listWidgets.map((widget) => <WidgetState key={widget.key} widget={widget} result={resultFor(widget)} />)}</div>{!widgets.length && <EmptyState title="No dashboard widgets available">Ask an administrator for a read permission.</EmptyState>}{warningKey && projection && <Dialog open title="Scheduling conflicts detected" onClose={dismissWarning}><p className="gf-form-error" role="alert">{projection.conflicts?.length ?? 0} exclusive-resource conflict{projection.conflicts?.length === 1 ? '' : 's'} were found in the seven-day projection.</p>{projectionIsStale(projection.calculatedAt) && <p className="gf-stale-warning" role="status">This report is older than one hour; the last successful snapshot is shown.</p>}<ul>{(projection.conflicts ?? []).slice(0, 5).map((conflict) => <li key={conflict.id}>{conflict.resourceName} · {conflict.occurrences.length} affected occurrence{conflict.occurrences.length === 1 ? '' : 's'}</li>)}</ul><div className="gf-dialog-actions"><Link className="gf-button gf-button-secondary" to="/schedules?tab=gantt" onClick={dismissWarning}>View Scheduling Gantt</Link><Button variant="secondary" onClick={dismissWarning}>Dismiss</Button></div></Dialog>}</main>
}
