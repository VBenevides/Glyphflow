...
export function DashboardPage() {
  ...
  const calculatedAt = projection?.calculated_at
  ...
  return <main className="gf-content"><PageHeader title="Overview" description="A live view of the scheduler resources you can access." meta="Live scheduler data" refresh={<QueryRefresh query={[...results, projectionQuery]} />} /><div className="gf-metric-grid">{metricWidgets.map((widget) => <WidgetState key={widget.key} widget={widget} result={resultFor(widget)} />)}</div><div className="gf-dashboard-grid">{listWidgets.map((widget) => <WidgetState key={widget.key} widget={widget} result={resultFor(widget)} />)}</div>{!widgets.length && <EmptyState title="No dashboard widgets available">Ask an administrator for a read permission.</EmptyState>}{warningKey && projection && <Dialog open title="Scheduling conflicts detected" onClose={dismissWarning}><p className="gf-form-error" role="alert">{projection.conflicts?.length ?? 0} exclusive-resource conflict{projection.conflicts?.length === 1 ? '' : 's'} were found in the seven-day projection.</p>{projectionIsStale(projection.calculated_at) && <p className="gf-stale-warning" role="status">This report is older than one hour; the last successful snapshot is shown.</p>}<ul>{(projection.conflicts ?? []).slice(0, 5).map((conflict) => <li key={conflict.id}>{conflict.resource_name} · {conflict.occurrences.length} affected occurrence{conflict.occurrences.length === 1 ? '' : 's'}</li>)}</ul><div className="gf-dialog-actions"><Link className="gf-button gf-button-secondary" to="/schedules?tab=gantt" onClick={dismissWarning}>View Scheduling Gantt</Link><Button variant="secondary" onClick={dismissWarning}>Dismiss</Button></div></Dialog>}</main>
}
