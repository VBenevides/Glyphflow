...
export function describeError(error: unknown): ErrorView {
  ...
  if (error.status === 400) return { title: 'Check the form', message: error.message, status: 400, fields: error.fields, retryable: false, correlationId: error.correlationId }
  if (error.status === 409) return { title: 'State conflict', message: error.message, status: 409, fields: error.fields, retryable: false, correlationId: error.correlationId }
