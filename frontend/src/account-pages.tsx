...
export function AccountPage() {
  ...
  return <main className="gf-content"><PageHeader title="Account" description="Manage your profile, login methods, and active sessions." action={<nav className="gf-account-tabs" aria-label="Account sections"><Link className={section === 'profile' || section === 'account' ? 'is-active' : ''} to="/account">Profile</Link><Link className={section === 'password' ? 'is-active' : ''} to="/account/password">Password</Link><Link className={section === 'identities' ? 'is-active' : ''} to="/account/identities">Identities</Link><Link className={section === 'sessions' ? 'is-active' : ''} to="/account/sessions">Sessions</Link></nav>} />
    <DataState query={query}>{(profile) => <>
      ...
      {(section === 'sessions' || section === 'account') && <section className="gf-card-panel"><h2>Owned sessions</h2>{sessions.length ? <ul className="gf-dashboard-list">{sessions.map((session) => <li key={session.id}><span><strong>{session.current ? 'Current session' : session.userAgent ?? 'Session'}</strong><br /><small>{formatDateTime(session.lastSeenAt ?? session.createdAt)} · <StatusPill status={session.current ? 'active' : 'session'} /></small></span><DangerousAction label={session.current ? 'Sign out' : 'Revoke'} warning={session.current ? 'Sign out of this browser?' : 'Revoke this session?' } onConfirm={() => revoke(session.id, Boolean(session.current))} /></li>)}</ul> : <p className="gf-muted">No active sessions were returned.</p>}</section>}
    </>}</DataState>
  </main>
}
