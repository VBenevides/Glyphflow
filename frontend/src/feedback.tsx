...
export function StartupPage({ status = 'Starting control plane…' }: Readonly<{ status?: string }>) {
...
export function FatalErrorPage({ title = 'Glyphflow cannot start', message, onRetry }: Readonly<{ title?: string; message: string; onRetry?: () => void }>) {
...
export function LoginRequiredPage({ onLogin }: Readonly<{ onLogin: () => void }>) {
