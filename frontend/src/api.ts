...
export type ScheduleProjectionOccurrence = { id: string; schedule_id: string; schedule_name: string; schedule_version_id: string; task_id: string; task_name: string; task_version_id: string; timezone: string; lane_id: string; lane_label: string; start_at: string; end_at: string }
export type ScheduleProjectionSegment = ScheduleProjectionOccurrence & { occurrence_count: number; conflicted: boolean; exclusive_resources: ScheduleProjectionResource[] }
export type ScheduleProjectionConflict = { id: string; resource_id: string; resource_name: string; start_at: string; end_at: string; occurrences: ScheduleProjectionOccurrence[] }
export type ScheduleProjection = { available: boolean; calculated_at?: string; window_start?: string; window_end?: string; duration_source?: string; segments?: ScheduleProjectionSegment[]; conflicts?: ScheduleProjectionConflict[] }
...
export type Run = { id: string; taskId?: string; taskVersionId?: string; scheduleId?: string; scheduleVersionId?: string; taskName?: string; state: string; placementBlocker?: string; attempt?: number; exitCode?: number; exitCodeMeaning?: string; error?: string; runner?: string; trigger?: string; scheduledFor?: string; maxMemoryUsedBytes?: number; averageMemoryUsedBytes?: number; duration?: number; attempts?: RunAttempt[]; events?: RunEvent[]; sessions?: RunSession[]; leases?: RunLease[]; cancellation?: RunCancellation; logGaps?: RunLogGap[] }
export type ExitCode = { code: number; meaning: string; isSystem?: boolean }
export type RunnerMetric = { sampledAt: string; cpuPercent: number; memoryPercent: number; memoryUsedBytes?: number; memoryTotalBytes?: number }
export type RunnerMetricHistory = { items: RunnerMetric[]; from: string; to: string }
export type Runner = { id: string; name: string; poolId?: string; desiredState?: string; observedState?: string; pool?: string; capacity?: number; currentCapacity?: number; activeCount?: number; heartbeatAt?: string; currentMetrics?: RunnerMetric; platform?: string; architecture?: string; natsEndpoint?: string; controlPlaneUrl?: string; isArchived?: boolean; isDeleted?: boolean }
