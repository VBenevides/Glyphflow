...
export type TaskVersion = { id: string; version: number; pool?: string; pinnedRunner?: string; command?: string[]; workingDirectory?: string; timeoutSeconds?: number; maxOutputBytes?: number; maxAttempts?: number; ambiguityPolicy?: string; executionSpecDigest?: string; createdAt?: string }
export type Task = { id: string; name: string; enabled?: boolean; state?: string; activeVersion?: number; pool?: string; pinnedRunner?: string; command?: string[]; workingDirectory?: string; placementSelectors?: Record<string, unknown>; environment?: Record<string, string>; secretReferences?: Record<string, unknown>; timeoutSeconds?: number; maxOutputBytes?: number; maxAttempts?: number; ambiguityPolicy?: string; latestRun?: Run }
...
export type Run = { id: string; taskId?: string; taskName?: string; state: string; attempt?: number; exitCode?: number; exitCodeMeaning?: string; runner?: string; trigger?: string; scheduledFor?: string; duration?: number; attempts?: RunAttempt[]; events?: RunEvent[]; sessions?: RunSession[]; leases?: RunLease[]; cancellation?: RunCancellation; logGaps?: RunLogGap[] }
export type ExitCode = { code: number; meaning: string; isSystem?: boolean }
export type Runner = { id: string; name: string; poolId?: string; desiredState?: string; observedState?: string; pool?: string; capacity?: number; currentCapacity?: number; activeCount?: number; heartbeatAt?: string; platform?: string; architecture?: string; isArchived?: boolean; isDeleted?: boolean }
