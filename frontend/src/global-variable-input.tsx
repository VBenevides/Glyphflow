...
export function GlobalVariableInput({ value, variables, onChange, multiline, className = '', ...props }: Readonly<Props>) {
  ...
  return <div className="gf-env-variable-picker">{input}{tooltip && <span className="gf-env-variable-tooltip" role="tooltip">{tooltip}</span>}{open && options.length > 0 && <div className="gf-task-options" role="listbox">{options.map((item) => <button key={item.id} type="button" className="gf-task-option" role="option" aria-selected={match?.[1] === item.name} title={item.value} onMouseDown={(event) => { event.preventDefault(); choose(item.name) }}><strong>$ENV:{item.name}</strong><small>{item.value}</small></button>)}</div>}</div>
}
