...
const groups: Group[] = [
  ...
  { name: 'Infrastructure', icon: Server, paths: ['/runners', '/resources'] },
...
export function Shell({ children }: { children: ReactNode }) {
  ...
  const navigation = <nav className="gf-sidebar-nav" aria-label="Primary navigation">{grouped.map(({ group, routes }) => { const Icon = group.icon; const expanded = openGroups[group.name] ?? true; return <section key={group.name} className="gf-nav-group"><button className="gf-nav-group-button" aria-expanded={expanded} onClick={() => setOpenGroups((current) => ({ ...current, [group.name]: !expanded }))}><Icon size={16} aria-hidden="true" /><span>{group.name}</span>{expanded ? <ChevronDown size={15} aria-hidden="true" /> : <ChevronRight size={15} aria-hidden="true" />}</button>{expanded && routes.map((route) => { const RouteIcon = routeIcon(route.path); return <NavLink key={route.path} to={route.path} end={route.path === '/'} className={({ isActive }) => `gf-nav-link${isActive ? ' is-active' : ''}`} title={collapsed ? route.label : undefined}><RouteIcon size={16} aria-hidden="true" /><span>{route.label}</span></NavLink> })}</section> })}</nav>
  const sidebar = <aside className={`gf-sidebar${collapsed ? ' is-collapsed' : ''}${mobileOpen ? ' is-mobile-open' : ''}`} aria-label="Glyphflow sidebar"><div className="gf-sidebar-brand"><div className="gf-brand-mark"><span>G</span></div><div className="gf-sidebar-brand-copy"><strong>Glyphflow</strong><small>Scheduler console</small></div>{mobileOpen && <Button variant="ghost" aria-label="Close navigation" onClick={() => setMobileOpen(false)}><X size={18} /></Button>}</div>{navigation}<div className="gf-sidebar-footer"><div className="gf-user-card"><Users size={18} aria-hidden="true" /><span><strong>{profile?.displayName ?? profile?.username}</strong><small>{profile?.username}</small></span></div><div className="gf-sidebar-actions"><Button variant="ghost" aria-label="Toggle theme" onClick={toggleTheme}>{theme === 'dark' ? <Sun size={17} /> : <Moon size={17} />}</Button><Button variant="ghost" aria-label="Sign out" onClick={logout}><LogOut size={17} /></Button></div></div></aside>
  return <div className="gf-app-shell"><Button ref={menuButtonRef} className="gf-mobile-menu" variant="secondary" aria-label="Open navigation" onClick={() => setMobileOpen(true)}><Menu size={18} /></Button>{mobileOpen && <button className="gf-drawer-scrim" aria-label="Close navigation" onClick={() => setMobileOpen(false)} />}{sidebar}<main id="app-main" className="gf-main" tabIndex={-1}>{children}</main><Button className="gf-collapse-button" variant="secondary" aria-label={collapsed ? 'Expand sidebar' : 'Collapse sidebar'} onClick={() => setCollapsed((value) => !value)}>{collapsed ? <PanelLeftOpen size={18} /> : <PanelLeftClose size={18} />}</Button></div>
}
