...
export function TaskDetailPage() {
  ...
  const canManage = hasPermission(permissions, 'tasks.manage')
  return <main className="gf-content"><QueryState query={query}>{(task) => <><PageHeader title={task.name} description="Immutable task versions and execution policy." action={canManage && <Button onClick={() => navigate(`/tasks/${taskId}/edit`)}>Edit version</Button>} /><div className="gf-metric-grid"><MetricCard label="Active version" value={task.activeVersion ?? '—'} /><MetricCard label="Runner pool" value={task.pool ?? '—'} /><MetricCard label="Timeout" value={task.timeoutSeconds ? `${task.timeoutSeconds}s` : '—'} /></div><section className="gf-card-panel"><h2>Related records</h2><nav className="gf-related-links"><Link to={links.schedules}>Schedules</Link><Link to={links.runs}>Runs</Link><Link to={links.audit}>Audit events</Link></nav></section><section className="gf-card-panel"><h2>Version history</h2><p className="gf-muted">Versions are immutable after publication. Select a version to review its diff.</p></section></>}</QueryState></main>
}
