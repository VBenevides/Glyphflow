...
import { Archive, CircleOff, ListChecks } from 'lucide-react'
import { useAuth } from './auth'
import { api, type Page, type Task, type TaskVersion } from './api'
import { DangerousAction } from './actions'
import { Button, DataTable, EmptyState, FilterInput, Input, MetricCard, PageHeader, Pagination, StatusPill } from './components'
...
export function TaskInventoryPage() {
  ...
  return <main className="gf-content"><PageHeader title="Tasks" description="Versioned commands and their schedules." action={canManage && <Button onClick={() => setEditorOpen(true)}>Create task</Button>} /><div className="gf-metric-grid"><MetricCard label="Total tasks" value={summaryQuery.data?.total ?? '—'} detail="All configured tasks" icon={ListChecks} tone="info" /><MetricCard label="Disabled tasks" value={summaryQuery.data?.disabled ?? '—'} detail="Tasks not accepting work" icon={CircleOff} tone="warning" /><MetricCard label="Archived tasks" value={summaryQuery.data?.archived ?? '—'} detail="Archived task history" icon={Archive} tone="default" /></div><nav className="gf-account-tabs" aria-label="Task status"><Link className={!archived ? 'is-active' : ''} to="/tasks">Tasks</Link><Link className={archived ? 'is-active' : ''} to="/tasks?archived=true">Archived Tasks</Link></nav><div className="gf-filter-bar"><FilterInput label="Search" options={(query.data?.items ?? []).flatMap((task) => [task.name, task.id])} value={search} onChange={(value) => { setSearch(value); setPage(1) }} placeholder="Name or ID" /><label>State <select className="gf-input" value={state} onChange={(event) => { setState(event.target.value); setPage(1) }}><option value="">All</option><option value="enabled">Enabled</option><option value="disabled">Disabled</option></select></label></div><QueryState query={query} empty={archived ? 'No archived tasks.' : 'Create a task version to schedule work.'}>{(data) => { const tasks = data.items.filter((task) => taskStateMatches(task, state)); return tasks.length ? <><DataTable caption={archived ? 'Archived tasks' : 'Tasks'} rows={tasks} columns={[{ key: 'name', label: 'Task', render: (task) => <Link to={`/tasks/${task.id}`}>{task.name}</Link> }, { key: 'id', label: 'Task ID', render: (task) => <code>{task.id}</code> }, { key: 'enabled', label: 'State', render: (task) => <StatusPill status={task.enabled === false ? 'disabled' : 'enabled'} /> }, { key: 'activeVersion', label: 'Version' }, { key: 'pool', label: 'Runner pool' }, { key: 'latestRun', label: 'Latest run', render: (task) => task.latestRun ? <StatusPill status={task.latestRun.state} /> : '—' }]} /><Pagination page={data.page} pages={data.pages ?? (data.total ? Math.ceil(data.total / data.limit) : 1)} limit={limit} onChange={setPage} onLimitChange={(next) => { setLimit(next); setPage(1) }} /></> : <EmptyState title={archived ? 'No archived tasks' : 'No matching tasks'}>{state ? `No ${state} tasks match this filter.` : 'Create a task version to schedule work.'}</EmptyState> }}</QueryState>{editorOpen && <TaskEditorPage inDialog onClose={() => setEditorOpen(false)} onSaved={async () => { setEditorOpen(false); await refresh() }} />}</main>
}
...
export function taskVersionDiff(previous: TaskVersion, current: TaskVersion): TaskVersionDiff[] {
  const rows: TaskVersionDiff[] = [
    ...
    { id: 'timeout', field: 'Execution Timeout Seconds', previous: previous.timeoutSeconds === undefined ? '—' : `${previous.timeoutSeconds}s`, current: current.timeoutSeconds === undefined ? '—' : `${current.timeoutSeconds}s` },
