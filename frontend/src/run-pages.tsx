...
export function RunInventoryPage() {
  const { permissions } = useAuth(); const [filters, setFilters] = useState({ task: '', runner: '', state: '', trigger: '', from: '', to: '' }); const [page, setPage] = useState(1); const [manualRunOpen, setManualRunOpen] = useState(false)
  ...
  return <main className="gf-content"><PageHeader title="Runs" description="Inspect attempts, state transitions, and external effects." action={hasPermission(permissions, 'runs.execute') && <Button onClick={() => setManualRunOpen(true)}>Start manual run</Button>} /><div className="gf-filter-bar"><TaskPicker value={filters.task} onChange={(value) => update('task', value)} label="Task" /><label>Runner<Input value={filters.runner} onChange={(event) => update('runner', event.target.value)} /></label><label>State<select className="gf-input" value={filters.state} onChange={(event) => update('state', event.target.value)}><option value="">All</option>{['WAITING', 'DISPATCHED', 'RUNNING', 'RETRY_WAIT', 'CANCELLING', 'SUCCEEDED', 'FAILED', 'TIMED_OUT', 'CANCELLED', 'UNKNOWN'].map((state) => <option key={state}>{state}</option>)}</select></label><label>Trigger<select className="gf-input" value={filters.trigger} onChange={(event) => update('trigger', event.target.value)}><option value="">All</option><option>SCHEDULE</option><option>MANUAL</option><option>RETRY</option></select></label><label>From<input className="gf-input" type="datetime-local" value={filters.from} onChange={(event) => update('from', event.target.value)} /></label><label>To<input className="gf-input" type="datetime-local" value={filters.to} onChange={(event) => update('to', event.target.value)} /></label></div><QueryState query={query} empty="No runs match these filters.">{(data) => data.items.length ? <><DataTable caption="Runs" rows={data.items} columns={[{ key: 'id', label: 'Run', render: (run) => <RunIDCell id={run.id} /> }, { key: 'taskName', label: 'Task', render: (run) => run.taskName ?? run.taskId ?? '—' }, { key: 'trigger', label: 'Trigger' }, { key: 'state', label: 'State', render: (run) => <StatusPill status={runStatusLabel(run.state)} /> }, { key: 'exitCode', label: 'Exit Code', render: (run) => run.exitCode ?? '—' }, { key: 'attempt', label: 'Attempt' }, { key: 'runner', label: 'Runner' }, { key: 'scheduledFor', label: 'Scheduled' }]} /><Pagination page={data.page} pages={data.pages ?? 1} onChange={setPage} /></> : <EmptyState title="No matching runs">Try a wider time range or remove a filter.</EmptyState>}</QueryState>{manualRunOpen && <ManualRunPage inDialog onClose={() => setManualRunOpen(false)} onStarted={async () => { setManualRunOpen(false); await query.refetch() }} />}</main>
}
