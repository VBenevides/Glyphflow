...
export function RunDetailPage() {
  ...
  return <main className="gf-content"><QueryState query={query}>{(run) => <><PageHeader title={`Run ${run.id}`} description={`Task ${run.taskName ?? run.taskId ?? '—'} · ${run.trigger ?? '—'}`} /><section className="gf-metric-grid"><div className="gf-metric"><span>State</span><strong><StatusPill status={runStatusLabel(run.state)} /></strong></div><div className="gf-metric"><span>Attempt</span><strong>{run.attempt ?? '—'}</strong></div><div className="gf-metric"><span>Runner</span><strong>{run.runner ?? '—'}</strong></div></section><section className="gf-card-panel"><h2>Actions</h2><div className="gf-dialog-actions">{canCancel && <Button variant="danger" disabled={!['WAITING', 'RUNNING', 'RETRY_WAIT', 'CANCELLING'].includes(run.state)}>Cancel</Button>}{canRetry && <Button variant="secondary" disabled={!['FAILED', 'TIMED_OUT', 'UNKNOWN'].includes(run.state)}>Retry</Button>}</div></section><section className="gf-card-panel"><h2>Attempts and events</h2><p className="gf-muted">Attempt, runner-session, lease, and state-event details are loaded from the run API.</p></section></>}</QueryState></main>
}
