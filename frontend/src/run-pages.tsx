...
import { Button, DataTable, Dialog, EmptyState, FilterInput, Input, PageHeader, Pagination, StatusPill } from './components'
...
export function RunInventoryPage() {
  ...
  const optionsQuery = useQuery({ queryKey: ['run-filter-options'], queryFn: ({ signal }) => api.get<Page<Run>>('/api/v1/runs', { all: true }, signal) })
  const runOptions = optionsQuery.data?.items ?? query.data?.items ?? []
  const update = (key: keyof typeof filters, value: string) => { setFilters((current) => ({ ...current, [key]: value })); setPage(1) }
  return <main className="gf-content"><PageHeader title="Runs" description="Inspect attempts, state transitions, and external effects." action={hasPermission(permissions, 'runs.execute') && <Button onClick={() => setManualRunOpen(true)}>Start manual run</Button>} /><div className="gf-filter-bar"><TaskPicker value={filters.task} onChange={(value) => update('task', value)} label="Task" /><FilterInput label="Runner" options={runOptions.flatMap((run) => run.runner ? [run.runner] : [])} value={filters.runner} onChange={(value) => update('runner', value)} /><label>State<select className="gf-input" value={filters.state} onChange={(event) => update('state', event.target.value)}><option value="">All</option>{['WAITING', 'DISPATCHED', 'RUNNING', 'RETRY_WAIT', 'CANCELLING', 'SUCCEEDED', 'FAILED', 'TIMED_OUT', 'CANCELLED', 'UNKNOWN'].map((state) => <option key={state}>{state}</option>)}</select></label><label>Trigger<select className="gf-input" value={filters.trigger} onChange={(event) => update('trigger', event.target.value)}><option value="">All</option><option>SCHEDULE</option><option>MANUAL</option><option>RETRY</option></select></label><label>From (UTC)<Input data-utc-datetime value={filters.from} placeholder="YYYY-mm-dd HH:MM UTC" onChange={(event) => update('from', event.target.value)} /></label><label>To (UTC)<Input data-utc-datetime value={filters.to} placeholder="YYYY-mm-dd HH:MM UTC" onChange={(event) => update('to', event.target.value)} /></label></div><QueryState query={query} empty="No runs match these filters.">{(data) => data.items.length ? <><DataTable caption="Runs" rows={data.items} columns={[{ key: 'id', label: 'Run', render: (run) => <RunIDCell id={run.id} /> }, { key: 'taskName', label: 'Task', render: (run) => run.taskName ?? run.taskId ?? '—' }, { key: 'trigger', label: 'Trigger' }, { key: 'state', label: 'State', render: (run) => <StatusPill status={runStatusLabel(run.state)} /> }, { key: 'exitCode', label: 'Exit Code', render: (run) => run.exitCode ?? '—' }, { key: 'attempt', label: 'Attempt' }, { key: 'runner', label: 'Runner' }, { key: 'scheduledFor', label: 'Scheduled', render: (run) => formatDateTime(run.scheduledFor) }]} /><Pagination page={data.page} pages={data.pages ?? 1} limit={limit} onChange={setPage} onLimitChange={(next) => { setLimit(next); setPage(1) }} /></> : <EmptyState title="No matching runs">Try a wider time range or remove a filter.</EmptyState>}</QueryState>{manualRunOpen && <ManualRunPage inDialog onClose={() => setManualRunOpen(false)} onStarted={async () => { setManualRunOpen(false); await query.refetch() }} />}</main>
}
