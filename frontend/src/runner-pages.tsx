...
import { Archive, CircleOff, Server } from 'lucide-react'
...
export function RunnerInventoryPage({ view = 'runners' }: { view?: 'runners' | 'pools' } = {}) {
  ...
  const summaryQuery = useQuery({ queryKey: ['runner-summary'], queryFn: async ({ signal }) => { const [all, disabled, archivedRunners] = await Promise.all([api.get<Page<Runner>>('/api/v1/runners', { page: 1, limit: 1 }, signal), api.get<Page<Runner>>('/api/v1/runners', { page: 1, limit: 1, desired_state: 'DISABLED' }, signal), api.get<Page<Runner>>('/api/v1/runners', { page: 1, limit: 1, archived: true }, signal)]); return { total: all.total ?? 0, disabled: disabled.total ?? 0, archived: archivedRunners.total ?? 0 } }, enabled: view === 'runners', refetchInterval: 5_000 })
          ...
          <>
            ...
            <section className="gf-card-panel">
              ...
              <QueryState query={currentRuns} empty="No current runs.">
                {(data) => data.items.length ? <><DataTable caption="Current runs" rows={data.items} columns={[{ key: 'id', label: 'Run', render: (run) => <RunIDCell id={run.id} compact /> }, { key: 'task', label: 'Task', render: (run) => run.taskName ?? run.taskId ?? '—' }, { key: 'state', label: 'State', render: (run) => <StatusPill status={run.state} /> }, { key: 'attempt', label: 'Attempt' }, { key: 'scheduled', label: 'Scheduled', render: (run) => run.scheduledFor ?? '—' }]} /><Pagination page={data.page} pages={data.pages ?? 1} limit={runLimit} onChange={setRunPage} onLimitChange={(next) => { setRunLimit(next); setRunPage(1) }} /></> : null}
              </QueryState>
            </section>
          </>
        )}
