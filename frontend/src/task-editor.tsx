...
export type TaskDraft = { name: string; command: string; workingDirectory: string; pool: string; pinnedRunner: string; selectors: KeyValueEntry[]; environment: EnvironmentEntry[]; resources: string[]; timeoutSeconds: string; maxOutputBytes: string; maxAttempts: string; ambiguityPolicy: string }

export const emptyTaskDraft: TaskDraft = { name: '', command: '', workingDirectory: '', pool: '', pinnedRunner: '', selectors: [], environment: [], resources: [], timeoutSeconds: '300', maxOutputBytes: '1048576', maxAttempts: '1', ambiguityPolicy: 'REQUIRE_MANUAL_RECONCILIATION' }
...
function ResourceEditor({ selected, resources, errors, onChange, onAdd, onRemove }: { selected: string[]; resources: Resource[]; errors: Record<string, string>; onChange: (index: number, value: string) => void; onAdd: () => void; onRemove: (index: number) => void }) {
  return <fieldset><legend>Resources</legend><div className="gf-table-wrap"><table className="gf-table"><caption className="gf-sr-only">Task resources</caption><thead><tr><th scope="col">Resource</th><th scope="col">Type</th><th scope="col"><span className="gf-sr-only">Actions</span></th></tr></thead><tbody>{selected.map((resourceID, index) => { const resource = resources.find((item) => item.id === resourceID); return <tr key={`${resourceID}-${index}`}><td><select className="gf-input" aria-label={`Task resource ${index + 1}`} value={resourceID} onChange={(event) => onChange(index, event.target.value)} aria-invalid={Boolean(errors[`resources.${index}`])}><option value="">Select a resource</option>{resources.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><FieldError message={errors[`resources.${index}`]} /></td><td>{resource?.kind?.toLowerCase() === 'exclusive' || !resource?.kind ? 'Exclusive' : resource.kind}</td><td><Button type="button" variant="ghost" onClick={() => onRemove(index)}>Remove</Button></td></tr> })}</tbody></table></div>{!resources.length && <p className="gf-muted">No resources are available.</p>}<Button type="button" variant="secondary" onClick={onAdd}>Add resource</Button></fieldset>
}
...
export function TaskEditorPage({ editTaskId, inDialog = false, onClose, onSaved }: TaskEditorProps = {}) {
  ...
  return inDialog ? <Dialog open title={title} onClose={close}>{form}</Dialog> : <main className="gf-content"><PageHeader title={title} description="Versions are immutable after publication." />{form}</main>
}
