...
export type TaskDraft = { name: string; command: string; workingDirectory: string; pool: string; pinnedRunner: string; selectors: string; environment: EnvironmentEntry[]; secretReferences: string; timeoutSeconds: string; maxOutputBytes: string; maxAttempts: string; ambiguityPolicy: string }
...
export function validateTaskDraft(draft: TaskDraft): Record<string, string> {
  ...
  try { JSON.parse(draft.selectors) } catch { errors.selectors = 'Enter valid JSON.' }
  try { JSON.parse(draft.secretReferences) } catch { errors.secretReferences = 'Enter valid JSON.' }
