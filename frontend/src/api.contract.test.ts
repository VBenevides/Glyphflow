...
describe('task API contract', () => {
  it('sends the backend request shape and reads the response shape', async () => {
    const input = { name: 'demo', command: ['echo', 'ok'], runner_pool: 'default', timeout_seconds: 30 }
    vi.stubGlobal('fetch', vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
      expect(JSON.parse(String(init?.body))).toEqual(input)
      return new Response(JSON.stringify({ id: 'task-1', name: 'demo', pool: 'default', timeoutSeconds: 30 }), { status: 201, headers: { 'content-type': 'application/json' } })
    }))
