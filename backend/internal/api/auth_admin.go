...
func (s Server) authAdminRoutes(mux routeRegistrar) {
	...
	mux.Handle("/api/v1/admin/auth/settings", s.require("auth.settings.manage", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if s.AuditQuery != nil {
			...
			s.AuditQuery.Add(AuditEvent{Actor: claims.UserID, ActorName: actorName, ActorEmail: actorEmail, Action: "auth.settings.updated", Description: auditDescription(r.Method, r.URL.Path), Target: r.URL.Path, Result: "success", CorrelationID: r.Header.Get("X-Correlation-ID"), Before: before, After: after, Input: map[string]any{"enabled": in.Enabled, "registration": in.Registration, "defaultRole": in.DefaultRole}, Output: map[string]any{"passwordLoginEnabled": in.Enabled, "registrationEnabled": in.Registration, "defaultRole": in.DefaultRole}})
		}
