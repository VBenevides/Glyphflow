...
func TestCurrentUserReturnsProfileAndOwnSessionRevocation(t *testing.T) {
	...
	server := Server{Auth: sessions.Authenticator(), CurrentUser: &CurrentUserService{Sessions: sessions, Profile: func(c Claims) (map[string]any, error) {
		return map[string]any{"id": c.UserID, "permissions": []string{"tasks.read"}}, nil
	}}}
...
func TestCurrentUserUsesAuthServiceProfile(t *testing.T) {
	...
	user, _ := auth.Register("u@example.com", "correct horse")
	tokens, _ := auth.Login("u@example.com", "correct horse")
