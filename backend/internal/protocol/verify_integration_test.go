...
func TestVerifyOrderAndEventBeforeReplayAcceptance(t *testing.T) {
	...
	order := OrderPayload{Version: ProtocolVersion, OrderID: "order-1", RunID: "run-1", Attempt: 1, LeaseToken: "lease", RunnerID: "worker-1", IssuedAt: now.Add(-time.Second), NotBefore: now.Add(-time.Second), ExpiresAt: now.Add(time.Minute), Type: OrderExecute, Command: []string{"echo"}, WorkingDir: "/tmp", TimeoutSeconds: 1}
