...
func (s *OIDCAuthorizationStateStore) Create(ctx context.Context, state OIDCAuthorizationStateRecord) error {
	_, err := s.pool.Exec(ctx, `INSERT INTO sso_authorization_states (id, provider_id, state_hash, nonce_hash, encrypted_pkce_verifier, purpose, callback_url, expires_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, state.ID, state.ProviderID, state.StateHash, state.NonceHash, state.EncryptedPKCEVerifier, state.Purpose, state.Callback, state.ExpiresAt)
	return err
}
...
func (s *OIDCAuthorizationStateStore) Consume(ctx context.Context, stateHash, nonceHash, providerID, purpose, callback string, now time.Time) (OIDCAuthorizationStateRecord, error) {
	...
	err = tx.QueryRow(ctx, `SELECT id, provider_id, state_hash, nonce_hash, encrypted_pkce_verifier, purpose, callback_url, expires_at, consumed_at FROM sso_authorization_states WHERE state_hash = $1 FOR UPDATE`, stateHash).
		Scan(&state.ID, &state.ProviderID, &state.StateHash, &state.NonceHash, &state.EncryptedPKCEVerifier, &state.Purpose, &state.Callback, &state.ExpiresAt, &state.ConsumedAt)
	...
	if state.ConsumedAt != nil || state.NonceHash != nonceHash || (providerID != "" && state.ProviderID != providerID) || state.Purpose != purpose || (callback != "" && state.Callback != callback) || !now.Before(state.ExpiresAt) {
		return OIDCAuthorizationStateRecord{}, errors.New("authorization state is invalid")
	}
