...
func (s *AuditStore) Query(ctx context.Context, filter AuditFilter) ([]AuditEventRecord, AuditCounts, error) {
	...
	args := []any{filter.Actor, filter.Action, filter.Target, filter.Result, filter.CorrelationID, filter.ExcludeMethod, filter.ExcludeTarget, from, to, filter.ExcludeRunLogs}
	args = append(args, filter.Limit, (filter.Page-1)*filter.Limit)
	query += ` LIMIT $11 OFFSET $12`
