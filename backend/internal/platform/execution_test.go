...
func TestExecutionDigestAndLogBound(t *testing.T) {
	spec := ExecutionSpec{TaskVersion: "v1", Command: []string{"echo", "ok"}, WorkingDir: "/tmp", Duration: 10, MaxOutput: 4}
	digest, err := ExecutionDigest(spec)
	if err != nil || len(digest) != 64 {
		t.Fatalf("digest failed: %q %v", digest, err)
	}
	if other, _ := ExecutionDigest(ExecutionSpec{TaskVersion: "v1", Command: []string{"echo", "different"}, WorkingDir: "/tmp", Duration: 10, MaxOutput: 4}); digest == other {
